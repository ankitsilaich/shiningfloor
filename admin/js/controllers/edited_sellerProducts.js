app.controller('sellerEditedProducts', ['$scope', '$http','$stateParams','$q','$state','$log','$location','toaster', function($scope, $http, $stateParams,$q,$state,$log,$location,toaster) {
  $scope.numPages = 5;
  console.log('ss');
...
// API is different for differnet controller       
 $scope.requestToSearchAPI = function() {
       ...
       console.log(final);
      $http.get('../api/slim.php/shiningfloor/admin/sellers/editedproducts'+ final).then(function(resp) {
            $scope.product = resp.data.product_data;
            $scope.bigTotalItems = resp.data.totalResults;
            console.log($scope.bigTotalItems);
